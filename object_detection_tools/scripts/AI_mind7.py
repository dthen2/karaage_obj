...
import math
import numpy as np
import object_detection_ids5 as obj # 検出閾値にヒステリシス設けて安定化したバージョン
...
class Apply1:
    def __init__(self, memory_space, idnet, projectID, fields):
        ...
        f.time_resolution = 0.7 #sec
        f.space_resolution = 40. #pixel
    ...
    def app_main1(self):
        ...
        for object in self.object_list_previous:
            ...
            new_point.x = [object['x'],  450.-object['y']] 
            new_point.v = [object.get('Vx',0.0), -object.get('Vy',0.0)]
            new_point.radius = (object['size_x']+object['size_y'])/2.
...
class Work_in_app3:
    ...
    def __init__(self, time_org, dim, ID, x, idns, work_origin_ID, fieldID):
        self.workID = idns.register_ID({"data_type":"work", "time_stamp": time.time()})            # このアプリでは、単点毎にworkを生成する
        self.time_org = time_org        # 最新入力データのタイムスタンプ
        self.data_orgID = ID            # 入力データのID
        ...
        for data in self.aq_datas_list:
            for point in data.data.points: # データが少数点群である前提
                ...
                if new_work_created :
                    ...
                    new_work = Work_in_app3(data.time_stamp, point.dim, point.ID, x, self.idns, data.workID, data.fieldID)
                    self.works.append(new_work)
                    # 関係性の記述を忘れずに
...
class Apply7:
    ...
    def app_main7(self):
        ...
        for work in self.works : # work 毎に処理を走らせる
            ...
            # アップロードする新しいデータ作成
            if work.cogRelation.Attention > 0.5 or work.cogRelation.Contact: # Contact追加2022Apr6
                #print("Contact")
                relation_property_dat = {"data_type": "event", "projectID": self.projectID, "appID": self.appID, "workID": work.workID, "relation_IDs": []}
                strength = max(work.cogRelation.Attention, float(work.cogRelation.Contact))
                if work.cogRelation.Contact:
                    cogni_dict['Contact'] = 1.0
                    #print("Contact ID%d, %d" % (work.data_orgID1, work.data_orgID2))
                fromID = work.data_orgID1
                toID = work.data_orgID2
                self.idns.register_relation(fromID, toID, strength, "interaction", relation_property_dat)
                # 関係ではなく、イベントとしてデータ登録
