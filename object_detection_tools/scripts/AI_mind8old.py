...
class Apply1:
    def __init__(self, memory_space, idnet, projectID, fields):
        ...
        self.writer, self.scale_y, self.scale_x, self.sampling_time = obj.object_detection_init()
        f = fld.Field(idnet, fields, self.fieldID)  # fieldの登録 2022Mar25 追加
        f.time_resolution = max(0.7, self.sampling_time*20.) #sec
    ...
    def app_main1(self):
        ...
        for object in self.object_list_previous:
            ...
            if object.get("detected_class_label") != None:
                new_point.attribute.append(object["detected_class_label"])
        ...
        self.video_time += self.sampling_time
        return self.status
    ...
    def highestCogni(self):
        if type(self.data) == dict:
            ...
            for key in self.data.keys():
                if self.data[key] > maxval:
                    maxval = self.data[key]
                    keymax = key
            if maxval > 0.0:
                return keymax
            else:
...
class Apply4:
    ...
    def print_ID(self, work, x, radius, indicateID, attribute=[]):
        f_text = 'time=%.1f, ' % work.time_org
        text = 'ID:' + str(work.data_orgID)
        f_text += text + ', x=%.2f, y=%.2f' % (x[0], x[1])
        if attribute != []:
    ...
    def app_main4(self):
                ...
                if new_work_created:
                    ...
                    if len(data.participant_IDs) >= 2: # とりあえず、2点間の関係だけ扱う
                        ...
                        if ID1_found and ID2_found:
                            ...
                            new_work.fieldID = data.fieldID
                            new_work.data = data.data
                            #print(new_work.data_type)
                            self.works.append(new_work)
            if data.data_type == "point_cloud_small_num":
                for point in data.data.points: # データが少数点群である前提
                    new_work_created = True
        ...
        for work in self.works:
            ...
            # 2022Apr8 追加
                ...
                text_r = work.highestCogni()
                self.ax.text( (x1+x2)/2, (y1+y2)/2, text_r, size=10, color=work.color)
    ...
    def app_main5_infer(self):
        ...
        for work in self.works:
            ...
            if work.is_converge and work.mat >= matching_srsh_grad: 
                ...
                property_dat = {"data_type":  "point_cloud_large_num", "time_stamp": time.time(), \
                ...
                eval_data = memory.Data_On_Memory(self.projectID, self.appID, work.workID, work.fieldID, work.data_orgID)
                eval_data.data_type = "evaluation"
                category_txt = "Category ID is %-2d" % work.categoryID
                eval_data.data = {"onomatopoeia": category_txt}
                eval_data.time_stamp = time.time()
...
class Work_in_app6:
    ...
    def __init__(self, time_org, point, ID, idns, work_origin_ID, fields, fieldID):
        ...
        field = fields.fields_dic[fieldID]
        self.work_origin_ID = work_origin_ID
        self.cogObj = cog.Detected_Object(point, field.time_resolution)
...
class Work_in_app7:
    ...
    def __init__(self, time_org1, point1, ID1, work_origin_ID1,     \
        ...
        self.workID = idns.register_ID({"data_type":"work", "time_stamp": time.time()})            # このアプリでは、単点の組み合わせ毎にworkを生成する
        ...
        self.cogObj1 = cog.Detected_Object(point1, self.filter_time)
        self.cogObj2 = cog.Detected_Object(point2, self.filter_time)
        ...
        self.cogRelation = cog.Relation(self.cogObj1, self.cogObj2, self.filter_time)
...
class Apply7:
    def __init__(self, memory_space, idnet, projectID, fields):
        ...
        self.projectID = projectID
        self.fields = fields
        self.appID = idnet.register_ID({"data_type":"app","time_stamp": time.time(),'additional_str':'Cognitive Lingistics for two points relation'})
        self.aq_datas_list = []     # メモリ空間から得たデータの置き場
        self.works = []
        # データの要求条件作成。ディクショナリのリストである
        # ここで、"projectID" と、"appID"(自分自身のID) の2つは絶対に必要なキーなので忘れないように
    ...
    def app_main7(self):
        ...
        for work in self.works : # work 毎に処理を走らせる
            # データ処理本体
            cogni_dict = work.cogRelation.relation_evaluation(time.time())
            # アップロードする新しいデータ作成
            if work.cogRelation.Attention > 0.5 :#or work.cogRelation.Contact: # Contact追加2022Apr6->ContactでAttention立てるようにしたので不要
                ...
                #    #print("Contact ID%d, %d" % (work.data_orgID1, work.data_orgID2))
                ...
                if work.event_ID == 0:
                    work.event_ID = self.idns.register_ID({"data_type":"event", "time_stamp": time.time()})
                updata = memory.Data_On_Memory(self.projectID, self.appID, work.workID, work.fieldID, work.event_ID)
                ...
                updata.time_stamp = work.cogRelation.Attention_time
                #print("time = %.2f" % (updata.time_stamp - time.time()))
