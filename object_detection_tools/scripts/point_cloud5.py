...
class PointCloud:
    ...
    def points_line_sort(self):
        ...
        self.dist_mean = dist_total/max(1,self.len) # 点群が空だった場合に備えてmax 2022Feb7
...
class Closest:
    def __init__(self, dimension, xs, ref ):
        ...
        if len(xs) > 1 and len(ref) > 1:
            ...
            for n in range(dimension):
                ...
                self.magnify.append(scale_fact)
    ...
    def coordinate_transformation(self):
        ...
        cos0 = cos*self.magnify[0]
        cos1 = cos*self.magnify[1]
        sin0 = sin*self.magnify[0]
        sin1 = sin*self.magnify[1] 
        for x in self.xs:
            # とりあえず、2次元の場合のみ作成
            new_x = create_dimensional_list(self.dimension, 0.0)
            new_x[0] = (x[0] + self.sift[0])*cos0 - (x[1] + self.sift[1])*sin1
            new_x[1] = (x[1] + self.sift[1])*cos1 + (x[0] + self.sift[0])*sin0
