...
# 無限ループの部分を関数化
#
def object_detection( writer, idnet, projects, object_list_previous, last_time):
    stutus = True
    if args.device == 'raspi_cam':
        cam.capture(stream, 'bgr', use_video_port=True)
        img = stream.array
    ...
    object_list_new = infer_velocity(object_list_new, object_list_previous, last_time, time_now, filt_time=0.6)
    objects_missed = corr_missed_position(objects_missed, last_time, time_now)
    object_list_previous = retain_missed_items(object_list_new, objects_missed, time_now, retain_time=3.0)
    ...
    if args.device == 'raspi_cam':
        stream.seek(0)
        stream.truncate()
    return stutus, object_list_previous
