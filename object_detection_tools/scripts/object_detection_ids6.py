...
# AI_mind2.py で使えるように改造したもの
# 2022Apr27 クラス違いを100ピクセルズレと等価と見なす処理追加
# 2022Apr18 Ver.6 検出閾値にヒステリシス設けて安定化した
# 2022Apr8 Ver.5 新設。 初回検出時のみ低い検出スコアのアイテムを除去し、安定性を高めた
# また、無意味になった速度情報の表示を廃止
import argparse
import cv2
import numpy as np
import os
    ...
    for i in range(output_dict['num_detections']):
        ...
        if detection_score > score_sh:
            ...
            object_list_new.append({"x": x_object, "y": y_object, "size_x": size_x_object, "size_y": size_y_object,\
                  ...
                  "detected_class_id": class_id }) # 2022Apr27 追加
...
# 2022Apr8 新設 低い検出スコアのアイテムを除去
score_sh_final = 0.6
def remove_lowdetectionscoreItem(object_list_new):
    for n in reversed(range(len(object_list_new))):
        if object_list_new[n]['detection_score'] < score_sh_final:
            del object_list_new[n]
    return object_list_new

# ID付与 
# accept_ratio が大きいほど大きな位置ズレを許容
def give_IDs(idnet, projects, object_list_new, object_list_previous, time_now, accept_ratio, class_factor):
    ...
    for prv_item in object_list_previous:
        ...
        for item in object_list_new:
            ...
            # クラス違いを200ピクセルズレと等価と見なす処理追加. 100->200 2022Oct27
            dc  = min(1, max(-1, item["detected_class_id"] - prv_item["detected_class_id"]))
            distance_2 = dx**2 + dy**2 + dsx**2 + dsy**2 + abs(dc)*class_factor
            ...
            mutch = {"dist_ratio": distance_2/acceptable_d2, 'No_new': i, "No_prev": p, \
                    'detection_score': max(item['detection_score'], prv_item['detection_score'])} # 2022Apr8 追加。マッチングしたモノについては高い検出スコアを維持
        ...
        if mutch['dist_ratio'] < accept_ratio:
            ...
            if object_list_new[i].get('assained',False) == False \
               and object_list_previous[p].get('Found', False) == False:
                ...
                object_list_new[i]['detection_score'] = mutch['detection_score']
    ...
    object_list_new = remove_lowdetectionscoreItem(object_list_new)
    for item in object_list_new:
...
def object_detection_init():
    ...
    return writer, y_size, x_size, sampling_time, labels # labels を2022Oct26追加

############################################
# 無限ループの部分を関数化
#
def object_detection( writer, idnet, projects, object_list_previous, last_time, onomatope_list, time_now):
    stutus = True
    if args.device == 'raspi_cam':
        cam.capture(stream, 'bgr', use_video_port=True)
        img = stream.array
    ...
    # 検出結果それぞれの特性取得
    #time_now = time.time()
    object_list_new = scan_detected_object(img, output_dict, time_now, score_sh=0.1)

    # 深田追加部分 ： ＩＤ付与
    object_list_new = remove_duplication(object_list_new, duplication_scale=0.1)
    object_list_new, objects_missed = give_IDs(idnet, projects, object_list_new, object_list_previous, time_now, accept_ratio=0.5, class_factor=40000.)
    object_list_new = infer_velocity(object_list_new, object_list_previous, last_time, time_now, filt_time=1.8) # filt_time 0.6->1.2->0.8->1.8
...
#    'num_detections', 'detection_boxes', 'detection_scores','detection_classes', 'detection_masks'
    ...
    #print(tf.shape(tensor_dict['detection_classes']))
    #print(tf.shape(image_tensor))
    #print(tf.print(detection_graph))
    print(tf.print(tf.get_default_graph()))
    #print(tf.print(tensor_dict['detection_scores']))

    LOG_DIR = './logs'
    with tf.summary.FileWriter(LOG_DIR, tf_sess.graph) as tfwriter:
        tfwriter.add_graph(tf.get_default_graph())
    ...
    writer, y_size, x_size, sampling_time, labels = object_detection_init()
    object_list_previous = []
    ono=[]
    last_time=time.time()
    while True:                                         #writer, idnet, projects, object_list_previous, last_time, onomatope_list, time_now):
        status, object_list_previous, last_time = object_detection(writer, idnet, projects, object_list_previous, last_time, ono,time_now=time.time())
        if status == False:
            break
