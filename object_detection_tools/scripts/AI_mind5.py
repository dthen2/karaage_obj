...
class Apply1:
    ...
    def app_main1(self):
        ...
        self.point_cloud = pcd.PointCloud(2, xs=[], points=new_points)
...
class Apply4:
    def __init__(self, memory_space, idnet, projectID):
        ...
        self.data_aqcuire_conditions = [{ "aq_data_types": ["point_cloud_small_num"], "project_ID": self.projectID, "app_ID": self.appID},\
                                        ...
                                        { "aq_data_types": ["point_cloud_large_num"], "project_ID": self.projectID, "app_ID": self.appID},\
    ...
    def print_ID(self, work, x, radius):
        ...
        self.ax.text(x[0]-x_sift,                           x[1]+y_sift*(radius+100), work.onomatopoeia, size=10, color=work.color)
    ...
    def app_main4(self):
        ...
        for data in self.aq_datas_list:
            ...
            if data.data_type == "time_series_point_cloud_large_num" or data.data_type == "point_cloud_large_num":
                ...
                if new_work_created :
                    ...
                    # 点群の起源を探り、色を同じにする
                    new_work = Work_in_app4(data.time_stamp, data.data.dim, data.data_ID, copy.copy(data.data), self.idns, "Cloud")
                    new_work.fieldID = data.fieldID
                    ...
                    if color_determined == False:
                        ...
                        new_work.cloud_orgID = data.data_ID
                    else:
        ...
        for work in self.works:
            if work.data_type=="Single_point": 
                ...
                # IDの記述
                self.print_ID(work, x, work.point.radius)
...
class Work_in_app5:
    def __init__(self, workID, time_org, dim, dataID, field, data_xs, work_origin_ID, data_orgType):
        self.workID = workID
        ...
        self.count2_max = 100 # グラジェントアプローチの上限回数
...
class Apply5:
    ...
    def __init__(self, memory_space, idnet, projectID, categories):
        ...
        self.data_aqcuire_conditions = [{ "aq_data_types": ["time_series_point_cloud_large_num",\
                                          ...
                                          "project_ID": self.projectID, "app_ID": self.appID}]
        self.memory_space.set_aq_conditions(self.data_aqcuire_conditions)
    ...
    def gradient_approach(self, search_num_g, work, closest):
        ...
        for i in range(search_num_g):
            ...
            for n in range(work.dim):
                work.d_x[n]   = work.x_scale*diff_x[n]/work.diff_x_scale*20.
                work.d_rot[n] = work.rot_scale*diff_rot[n]/work.diff_rot_scale*400.
                work.d_mag[n] = work.mag_scale*diff_mag[n]/work.diff_mag_scale*200.
    ...
    def app_main5_aq(self):
        ...
        if time_now > (self.time_last+self.sampling_time):
            ...
            for data in self.aq_datas_list:
                ...
                working = False
                for work in self.works:
                    if work.data_orgID == data.data_ID:
                        working = True
                        break
                if working == False:
                    workID = self.idns.register_ID({"data_type":"work", "time_stamp": time.time()})
                    newwork = Work_in_app5(workID, data.time_stamp, data.data.dim, data.data_ID, data.fieldID, copy.deepcopy(data.data.xs), data.work_ID, data.data_type)
    ...
    def app_main5_infer(self):
        ...
        for work in self.works:
            ...
            if work.mode == self.CATEGORY_FINDING:
                ...
                #    print('In category finding. CategoryNum= ' + str(work.categoryNum))
                ...
                if work.categoryNum >= 0 and len(work.data_xs) > 1:
                    if snum < 0:
                        #print(self.cats.categories[work.categoryNum]["prototypes"][pnum]["data"])
                        #xs_cat = self.cats.categories[work.categoryNum]["prototypes"][pnum]["data"].xs
                        work.ref_points = self.cats.categories[work.categoryNum]["prototypes"][pnum]["data"]
                        work.snum = snum
                    else:
                        #xs_cat = self.cats.categories[work.categoryNum]["stereotypes"][snum]["data"].xs
                        ...
                        work.pnum = pnum
                    work.closest = pcd.Closest(work.dim, work.data_xs, work.ref_points.xs )
                    cloud_tmp = pcd.PointCloud(2, xs=copy.deepcopy(work.data_xs), points=[])
                    #print('ID = ' + str(work.data_orgID))
                    #print('xs = '+str(work.data_xs))
                    #print('xs_cat = '+str(xs_cat))
                    #print(cloud_tmp.xs)
                    #print(cloud_tmp.far_most_num)
                    cloud_tmp.points_line_sort()
                    work.x_scale   = cloud_tmp.dist_mean*0.0001
                    if cloud_tmp.cloud_radius == 0.0:
                        print(work.data_xs)
                        print(cloud_tmp.xs)
                        print(cloud_tmp.cloud_radius)
                    work.rot_scale = cloud_tmp.dist_mean/cloud_tmp.cloud_radius*0.0001
                    work.mag_scale = cloud_tmp.dist_mean/cloud_tmp.cloud_radius*0.0001
                    work.resolution = cloud_tmp.dist_mean
                    # モード切替
                    work.mode = self.RANDOM_APPROACH
                    #if work.data_orgID==9:
                    #    print('Sift to random approach')
                else :
            ...
            if is_converge:
                ...
                relation_property_dat = {"data_type": work.data_orgType, "project_ID": self.projectID, "app_ID": self.appID, "work_ID": work.workID, "relation_IDs": relation_IDs}
                self.idns.register_relation(work.data_orgID,  self.cats.categories[work.categoryNum]["ID"], work.mat, "matched", relation_property_dat)
                # 一致検出したら、そのプロトタイプorステレオタイプをメモリー空間に投げる
                # 例外的に、IDを取得せず、カテゴリーのIDを与えているが・・・
                data_on_memory = memory.Data_On_Memory(self.projectID, self.appID, work.workID, work.fieldID, self.cats.categories[work.categoryNum]["ID"])
                ...
                sift = pcd.cloud_centre(work.data_xs, 2)
                ...
                #print("Category matching obtained")
                data = memory.Data_On_Memory(self.projectID, self.appID, work.workID, work.fieldID, work.data_orgID)
                data.data_type = "evaluation"
                category_txt = "Category ID is %-2d" % self.cats.categories[work.categoryNum]["ID"]
        ...
        # 200秒以上更新されなかったら、そのworkを除去する
        remove_unused_works(200., self.works)
