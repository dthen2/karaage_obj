...
import math
import numpy as np
import object_detection_ids6 as obj # 検出閾値にヒステリシス設けて安定化したバージョン
...
# 認知言語学っぽい認知のアップデートに合わせて、更新してVer.8 とした
# object_detection の仕様も少し変えて、カメラ画面サイズに合わせる仕様作った
#  Appl4における表示と、Appl3でのフーリエ評価の閾値で画面サイズ評価を使用
# Ver.7 に当たって、軌跡の点群をイベントで切断する機能追加や、イベントを表示する機能の強化を行った。
#  また、ファイルに記録する仕組みを追加 in Apply4
#  多数点群を軌跡として描く所を少し高速化（今までfor文を回してた）
...
def remove_unused_works(retain_time, works):
    for n in reversed(range(len(works))):
        if time_g(real_time) > works[n].time_org + retain_time:
            del works[n]
...
class Apply1:
    def __init__(self, memory_space, idnet, projectID, fields):
        self.writer, self.scale_y, self.scale_x, self.sampling_time, labels = obj.object_detection_init()
        self.labels = labels    # 2022Oct26
        if self.sampling_time==0.0: # カメラの場合
            self.real_time = True
            self.sampling_time=0.7
        else:
            self.real_time = False
        time_now = time_g(self.real_time)
        self.memory_space = memory_space
        self.idns = idnet
        self.projectID = projectID
        self.appID = idnet.register_ID({"data_type":"app","time_stamp": time_now,'additional_str':'Object detection'})
        self.workID = idnet.register_ID({"data_type":"work","time_stamp": time_now})
        self.fieldID = idnet.register_ID({"data_type":"field","time_stamp": time_now})
        self.dataID = idnet.register_ID({"data_type":"point_cloud_small_num","time_stamp": time_now})
        # object_detection のクラスを登録する機能 2022Oct26
        self.classID_top = idnet.IDs[-1] + 1
        for label in labels:
            idnet.register_ID({"data_type":"string","time_stamp": time_now,'additional_str':label})
        ...
        f.time_resolution = self.sampling_time #*20.) #sec
        f.space_resolution = 40.*(self.scale_x+self.scale_y)/(640. + 480.) #pixel
        f.max_x = self.scale_x
        f.max_y = self.scale_y
        self.object_list_previous = []
        self.status = False     # ESCキーが押されたことの判定
        self.point_cloud = pcd.PointCloud(2, xs=[], points=[])
        self.data_on_memory = memory.Data_On_Memory(projectID, self.appID, self.workID, self.fieldID, self.dataID,time_g(self.real_time))
        self.data_on_memory.work_origin_IDs = [self.workID]
        ### テスト用多数点群。「の」の字の多数点群
        self.point_cloud_large = pcd.PointCloud(2, xs=cat.SAMPLE_CLOUD2, points=[])
        self.point_cloud_large.ID = idnet.register_ID({"data_type":"point_cloud_large_num","time_stamp": time_now})
        dataID2 = idnet.register_ID({"data_type":"point_cloud_large_num","time_stamp": time_now})
        self.data_on_memory2 = memory.Data_On_Memory(projectID, self.appID, self.workID, self.fieldID, dataID2,time_g(self.real_time))
        ...
        self.last_time = time_now-0.1    # 2022Feb21

    
    def samp_time(self):
        return self.sampling_time, self.real_time
    ...
    def app_main1(self):
        ...
        self.status, self.object_list_previous, self.last_time = obj.object_detection(self.writer, self.idns, self.project, self.object_list_previous, last_time=self.last_time, onomatope_list=onomatope_list, time_now=time_g(real_time))
        new_points = []
        participant_IDs = []
        for object in self.object_list_previous:
            participant_IDs.append(object["ID"])
            new_point = pcd.Point_F(2, time_g(real_time))
            new_point.ID = object['ID']
            new_point.x = [object['x'],  self.scale_y-object['y']] 
            new_point.v = [object.get('Vx',0.0), -object.get('Vy',0.0)]
            new_point.radius = (object['size_x']+object['size_y'])/4. # radius なのに diameter相当の値を入れてたので修正（汗）
            if object.get("label") != None:
                new_point.attribute.append(object["label"])
            # 2022Oct26 クラスとの一致を関係性として登録
            relation_property_dat = {"data_type": "point", "projectID": self.projectID, "appID": self.appID, "workID": self.workID, "relation_IDs": []}
            registered_classID = object["detected_class_id"] + self.classID_top
            idnet.register_relation(object["ID"], registered_classID, 1., "matched", relation_property_dat)
            # 2022Oct26 クラスとの一致 end
            #print(idnet.properties[registered_classID]["additional_str"])
        ...
        self.data_on_memory.time_stamp = time_g(real_time)   # タイムスタンプ
        ...
        self.data_on_memory2.time_stamp = time_g(real_time)   # タイムスタンプ              
...
class Work_in_app2:
    def __init__(self, time_org, dim, ID, x, idns, work_origin_ID):
        self.workID = idns.register_ID({"data_type":"work", "time_stamp": time_g(real_time)})            # このアプリでは、単点毎にworkを生成する
...
class Apply2:
    ...
    def __init__(self, memory_space, idnet, projectID, fields):
        ...
        self.idns = idnet
        self.projectID = projectID
        self.appID = idnet.register_ID({"data_type":"app","time_stamp": time_g(real_time),'additional_str':'Point to time_series_cloud'})
        self.aq_datas_list = []     # メモリ空間から得たデータの置き場
        self.works = []
    ...
    def app_main2(self):
            ...
            if data.data_type == "point_cloud_small_num":
                for point in data.data.points: # データが少数点群である前提
                    ...
                    for work in self.works :
                        if point.ID == work.data_orgID: # 現有ID一致しなければ、new_work_createdがON・・・一つの例
                            ...
                            if work.event.get("Punctuality",0.) > 0.5 and \
                               time_g(real_time) - work.time_begin > self.event_dead_time and \
                               time_g(real_time) - work.event_time < self.event_dead_time:
                                new_work_created = True
                    ...
                    if new_work_created :
                        ...
                        new_work.time_begin = time_g(real_time)   # 2022Mar21 追加
        ...
        for work in self.works : # work 毎に処理を走らせる
            # データ処理本体
            # アップロードする新しいデータ作成
            data = memory.Data_On_Memory(self.projectID, self.appID, work.workID, work.fieldID, work.points.ID, time_g(real_time))
            data.data = work.points
            data.time_stamp = time_g(real_time)
...
class Work_in_app3:
    ...
    def __init__(self, time_org, dim, ID, x, idns, work_origin_ID, fields, fieldID):
        ...
        self.work_origin_ID = work_origin_ID
        self.flx = flt.Fourier(self.FREQUENCY_LIST, self.decay_time_list, time_new=time_g(real_time))
        self.fly = flt.Fourier(self.FREQUENCY_LIST, self.decay_time_list, time_new=time_g(real_time))
        self.plx = flt.Impulse(self.filt_time_list, filt_ratio=5.0, time_new=time_g(real_time))
        self.ply = flt.Impulse(self.filt_time_list, filt_ratio=5.0, time_new=time_g(real_time))
...
class Apply3:
    def __init__(self, memory_space, idnet, projectID, fields):
        ...
        self.idns = idnet
        self.projectID = projectID
        self.appID = idnet.register_ID({"data_type":"app","time_stamp": time_g(real_time),'additional_str':'Fourier examination'})
        self.aq_datas_list = []     # メモリ空間から得たデータの置き場
        self.works = []
        self.fields = fields
        # データの要求条件作成。ディクショナリのリストである
    ...
    def app_main3(self):
        ...
        for work in self.works : # work 毎に処理を走らせる
            # データ処理本体
            # アップロードする新しいデータ作成
            data = memory.Data_On_Memory(self.projectID, self.appID, work.workID, work.fieldID, work.data_orgID, time_g(real_time))
...
class Apply4:
    def __init__(self, memory_space, idnet, projectID, fields):
        ...
        self.idns = idnet
        self.projectID = projectID
        self.appID = idnet.register_ID({"data_type":"app","time_stamp": time_g(real_time),'additional_str':'Data drawing'})
        self.aq_datas_list = []     # メモリ空間から得たデータの置き場
        self.works = []
        self.fig = plt.figure()
    ...
    def print_ID(self, work, x, radius, indicateID, attribute=[]):
        f_text = 'time=%.2f, ' % work.time_org
        text = 'ID:' + str(work.data_orgID)
        f_text += text + ', x=%.2f, y=%.2f, r=%.2f' % (x[0], x[1], radius)
        if attribute != []:
            f_text += ', Class:%s' % str(attribute[0])
        x_sift = self.scale_x/60/2
        y_sift = self.scale_y/4000/2
        if indicateID:
        ...
        for event_key in work.event.keys():
            if work.event_time > time_g(real_time)-2.:
    ...
    def print_relation(self, work, text):
        text_f = 'time=%.2f' % work.time_org
    ...
    def app_main4(self):
        ...
        for work in self.works:
            if work.data_type=="Single_point": 
                ...
                # IDの記述
                self.print_ID(work, x, work.point.radius, True, work.point.attribute)
            ...
            # 2022Apr8 追加
                ...
                self.print_relation(work, text_r)
...
class Apply5:
    ...
    def __init__(self, memory_space, idnet, projectID, fields, categories):
        ...
        self.appID = idnet.register_ID({"data_type":"app","time_stamp": time_g(real_time),'additional_str':'Category mutching'})
        self.aq_datas_list = []     # メモリ空間から得たデータの置き場
        self.works = []
        # データの要求条件作成。ディクショナリのリストである
        # 少数点群も含めちゃうところがミソ
        ...
        self.time_last = time_g(real_time)
    ...
    def app_main5_aq(self):
        ...
        if time_now > (self.time_last+self.sampling_time):
            ...
            for data in self.aq_datas_list:
                ...
                if working == False:
                    workID = self.idns.register_ID({"data_type":"work", "time_stamp": time_g(real_time)})
    ...
    def app_main5_infer(self):
        ...
        for work in self.works:
            ...
            if work.is_converge and work.mat >= matching_srsh_grad: 
                ...
                property_dat = {"data_type":  "point_cloud_large_num", "time_stamp": time_g(real_time), \
                ...
                data_on_memory = memory.Data_On_Memory(self.projectID, self.appID, work.workID, work.fieldID, newID, time_g(real_time))
                data_on_memory.data_type = cat_data_type
                data_on_memory.time_stamp = time_g(real_time)
                ...
                eval_data = memory.Data_On_Memory(self.projectID, self.appID, work.workID, work.fieldID, work.data_orgID, time_g(real_time))
                eval_data.data_type = "evaluation"
                category_txt = "Category ID is %-2d" % work.categoryID
                eval_data.data = {"onomatopoeia": category_txt}
                eval_data.time_stamp = time_g(real_time)
...
class Work_in_app6:
    ...
    def __init__(self, time_org, point, ID, idns, work_origin_ID, fields, fieldID):
        ...
        field = fields.fields_dic[fieldID]
        self.work_origin_ID = work_origin_ID
        self.cogObj = cog.Detected_Object(point, min(0.7,field.time_resolution*20.), time_g(real_time))
        self.cogObj.STANDARD_SPEED = field.space_resolution*0.7/field.time_resolution # 今回の速度スケールに合わせたモノ
        self.cogObj.A_STANDARD = field.space_resolution*0.7/field.time_resolution     # 今回の速度スケールに合わせたモノ
...
class Apply6:
    def __init__(self, memory_space, idnet, projectID, fields):
        ...
        self.projectID = projectID
        self.fields = fields
        self.appID = idnet.register_ID({"data_type":"app","time_stamp": time_g(real_time),'additional_str':'Cognitive Lingistics for single point'})
        self.aq_datas_list = []     # メモリ空間から得たデータの置き場
        self.works = []
        # データの要求条件作成。ディクショナリのリストである
        # ここで、"projectID" と、"appID"(自分自身のID) の2つは絶対に必要なキーなので忘れないように
    ...
    def app_main6(self):
        ...
        for work in self.works : # work 毎に処理を走らせる
            ...
            if work.cogObj.Attention > 0.5:
                ...
                data.time_stamp = work.time_org #work.cogObj.Attention_time
...
class Work_in_app7:
    ...
    def __init__(self, time_org1, point1, ID1, work_origin_ID1,     \
        ...
        self.workID = idns.register_ID({"data_type":"work", "time_stamp": time_g(real_time)})            # このアプリでは、単点の組み合わせ毎にworkを生成する
        ...
        self.cogObj1 = cog.Detected_Object(point1, self.filter_time, time_g(real_time))
        self.cogObj2 = cog.Detected_Object(point2, self.filter_time, time_g(real_time))
        ...
        self.cogRelation = cog.Relation(self.cogObj1, self.cogObj2, self.filter_time, time_g(real_time))
...
class Apply7:
    def __init__(self, memory_space, idnet, projectID, fields):
        ...
        self.projectID = projectID
        self.fields = fields
        self.appID = idnet.register_ID({"data_type":"app","time_stamp": time_g(real_time),'additional_str':'Cognitive Lingistics for two points relation'})
        self.aq_datas_list = []     # メモリ空間から得たデータの置き場
        self.works = []
        # データの要求条件作成。ディクショナリのリストである
        # ここで、"projectID" と、"appID"(自分自身のID) の2つは絶対に必要なキーなので忘れないように
    ...
    def app_main7(self):
            ...
            for pointB in points:
                ...
                if new_work_created and point1["field"] == point2["field"] and point1["point"].ID != point2["point"].ID:
                    ...
                    new_work = Work_in_app7(point1["time"], point1["point"], point1["point"].ID, point1["workID"],\
                                            point2["time"], point2["point"], point2["point"].ID, point2["workID"], point1["field"], self.idns, self.fields)
                    self.works.append(new_work)
                    # work同士の関係性の記述必要か？
                    relation_property_dat = {"data_type": "work", "projectID": self.projectID, "appID": self.appID, "workID": new_work.workID, "relation_IDs": []}
        ...
        for work in self.works : # work 毎に処理を走らせる
            ...
            if work.cogRelation.Attention > 0.5 :#or work.cogRelation.Contact: # Contact追加2022Apr6->ContactでAttention立てるようにしたので不要
                ...
                if work.event_ID == 0:
                    work.event_ID = self.idns.register_ID({"data_type":"event", "time_stamp": time_g(real_time)})
                updata = memory.Data_On_Memory(self.projectID, self.appID, work.workID, work.fieldID, work.event_ID, time_g(real_time))
                ...
                updata.time_stamp = work.time_org # work.cogRelation.Attention_time
...
def init_category(idnet, projectID):
    ...
    for prototype in prototypes:
        ...
        id = idnet.register_ID({"data_type":"category","time_stamp": time.time()})
        type = { \
        ...
        categories.register_category(category_data, prototype["Description"], time.time())
    return categories
...
if __name__ == "__main__":
    ...
    sampling_time, real_time = apply1.samp_time()
    ...
    while(True): # 本当は、タイマー機能でやる。後ほど
        prev_time = time.time()
        memory_space.memory_space_manerger(time_g(real_time))
        memory_space.librarian_swapper_before_scanner()
        memory_space.librarian_scanner(time_g(real_time))
