...
#                   類似性（フカダ用語）・・・アルゴリズム未作成
    ...
    def relation_evaluation(self, objectA, objectB, time_now):
        ...
        self.calc_relative_motion()
        # 仕様変更。速度の大きさを入れる 2022June2
        cogni_vector = np.dot(self.relation_matrix, self.state_vector()) * (max(self.Vanor, self.Vbnor)/self.STANDARD_SPEED)
        ...
        self.relation["Agency"] = (max(0., self.cogni_dict["Chase"]))**2
        self.relation["Affectedness"] = (max(0., self.cogni_dict["Chased"]))**2
