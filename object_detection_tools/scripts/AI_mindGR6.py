...
TIME_RESOLUTION = 0.2 #sec
SPACE_RESOLUTION = 10.0 # pixel
STANDARD_SPEED = 20.0   # pixel/sec
REF_DISTANCE = 300.0    # pixel
...
def remove_unused_works(retain_time, works, file=False):
...
class Apply1:
    def __init__(self, memory_space, idnet, projectID, fields):
        ...
        f.ref_speed = STANDARD_SPEED            # 40 pixel/sec 2022June10
        f.ref_distance = REF_DISTANCE           # 300.0 #pixel 2022June10
...
class Work_in_app2:
    def __init__(self, time_org, dim, ID, x, idns, work_origin_ID, frame, file_head):
        ...
        filename = 'ai_p' + file_head + '%d_%d' % (frame, ID)   # 2022July1 追加
        filename += '.txt'                          # 2022July1 追加
        self.file = open(filename, 'a')             # 2022July1 追加
...
class Apply2:
    ...
    def app_main2(self, frame, status):
        # データ置き場を空にする
        self.aq_datas_list = []
        # 司書にデータ取得依頼をする
                        ...
                        if point.ID == work.data_orgID: # 現有ID一致しなければ、new_work_createdがON・・・一つの例
                            ...
                            #    #print("Cut Cut Cut ID=%d" % work.data_orgID)
                                ...
                                if work.time_org < data.time_stamp and work.is_new:
                                    ...
                                    # 2022July1 ファイルに書き出す仕様追加 
                                    text_f = '%d %d %.1f %.2f %.2f ' % (frame, point.ID, data.time_stamp, x[0], x[1])
                                    text_f += '\n'
                                    work.file.write(text_f)
                    if new_work_created :
                        # 新しい点を検出したら新しいworkを立ち上げ、新しい時系列多数点群を創成する
                        # 2022July1 追加。ファイルに残す
                        file_head = '%d_' % (point.attribute[0])
                        if x[0] < 650. :
                            file_head += 'l_'
                        elif x[0] > 800.  :
                            file_head += 'r_'
                        elif x[0] < 800. and x[0] > 650. :
                            file_head += 'm_'
                        else:
                            file_head += 'x_'
                        new_work = Work_in_app2(data.time_stamp, point.dim, point.ID, x, self.idns, data.workID, frame, file_head)
                        ...
                        # 切った点群の連続性を serise の関係性として登録 -> 切る仕様廃したのでここも廃止
                        #if previousID != 0:
                        ...
                        text_f = '%d %d %.1f %.2f %.2f ' % (frame, point.ID, data.time_stamp, x[0], x[1])
                        text_f += '\n'
                        new_work.file.write(text_f)
        # 5秒以上更新されなかったら、そのworkを除去する
        remove_unused_works(5., self.works, file=True)
...
class Work_in_app3:
    ...
    def __init__(self, time_org, dim, ID, x, idns, work_origin_ID, fields, fieldID):
        ...
        self.fl_min = 10.    # 検出閾値 2022Mar25 変更。2022June10再変更 40. -> 10.
        self.work_origin_ID = work_origin_ID
        ...
        for data in self.aq_datas_list:
            for point in data.data.points: # データが少数点群である前提
                ...
                if new_work_created :
                    ...
                    #print(data.time_stamp)
        # 更新されないデータは除去
        remove_unused_works(5., self.works)
...
class Work_in_app4_relation: # 2022June9 関係性をファイルに書き出すためのもの。GRファクトリー専用
    def __init__(self, time_org, eventID, ID1, ID2, point1, point2, idns, file_head, frame):
        self.time_event = time_org
        #self.time_eval1 = -1
        #self.time_eval2 = -1
        self.time_point1 = -1
        self.time_point2 = -1
        filename = 'ai_r' + file_head + '%d_%d_%d' % (frame, ID1, ID2)
        filename += '.txt'
        self.file = open(filename, 'a')
        self.eventID = eventID  # Work_in_app4 の data_orgID からもらってくる。
        self.ID1 = ID1
        self.ID2 = ID2
        self.point1 = point1
        self.point2 = point2
        ...
        self.onomatodict2 = {}
        self.event = {}
        self.time_file = -1     # 更新データのみファイルに書くため
        self.time_org = time_org
    
    def renew_relation(self, works):
        for work in works:
            #if work.time_org > self.time_org:
            if work.data_type == "Relation" and self.time_event < work.time_org:
                if self.eventID == work.data_orgID:
                    self.event = work.event
                    self.time_event = work.time_org
            # Work_in_app4のdata_typeにevaluationは無かった。バグ修正2022June23
            #elif work.data_type == "evaluation" and self.time_eval1 < work.time_org:
            #    if self.ID1 == work.data_orgID:
            #        self.onomatodict1 = work.onomatodict
            #        self.time_eval1 = work.time_org
            #elif work.data_type == "evaluation" and self.time_eval2 < work.time_org:
            #    if self.ID2 == work.data_orgID:
            #        self.onomatodict2 = work.onomatodict
            #        self.time_eval2 = work.time_org
            elif work.data_type == "Single_point":
                if self.ID1 == work.data_orgID and self.time_point1 < work.time_org:
                    self.point1 = work.point
                    self.onomatodict1 = work.onomatodict
                    self.time_point1 = work.time_org
                elif self.ID2 == work.data_orgID and self.time_point2 < work.time_org: # ここが書き込まれないバグ修正2022June30
                    self.point2 = work.point
                    self.onomatodict2 = work.onomatodict
                    self.time_point2 = work.time_org
        self.time_org = max(self.time_event, self.time_point1, self.time_point2)   # ここ悩ましい


    def write_event(self, frame):
        text_f = '%d %.2f ' % (frame, self.time_event)
        text_f += '%d %d %d ' % (self.eventID, self.ID1, self.ID2)
        text_f += '%d %d ' % (self.point1.attribute[0], self.point2.attribute[0])
        # event properties
        dist = math.sqrt((self.point1.x[0]-self.point2.x[0])**2 + (self.point1.x[1]-self.point2.x[1])**2)
        text_f += '%.2f ' % dist
        text_f += '%.2f ' % self.event.get("Contact", 0.0)
        text_f += '%.2f ' % self.event.get("Punctuality", 0.0)
        text_f += '%.2f ' % self.event.get("Kinesis", 0.0)
        text_f += '%.2f ' % self.event.get("Volitionality", 0.0)
        text_f += '%.2f ' % self.event.get("Agency", 0.0)
        text_f += '%.2f ' % self.event.get("Affectedness", 0.0)
        text_f += '%.2f ' % self.event.get("Linkageness", 0.0)
        text_f += '%.2f ' % self.event.get("Unity", 0.0)
        text_f += '%.2f ' % self.event.get("Identicality", 0.0)
        text_f += '%.2f ' % self.event.get("Similarity", 0.0)
        text_f += '%.2f ' % self.event.get("Hit", 0.0)
        text_f += '%.2f ' % self.event.get("Avoid", 0.0)
        text_f += '%.2f ' % self.event.get("Chase", 0.0)
        text_f += '%.2f ' % self.event.get("Chased", 0.0)
        text_f += '%.2f ' % self.event.get("Co_motion", 0.0)
        text_f += '%.2f ' % self.event.get("Sideby", 0.0)
        text_f += '%.2f ' % self.event.get("Attend", 0.0)
        text_f += '%.2f ' % self.event.get("Behind", 0.0)
        text_f += '%.2f ' % self.event.get("Catchup", 0.0)
        text_f += '%.2f ' % self.event.get("Meet", 0.0)
        text_f += '%.2f ' % self.event.get("Apart", 0.0)
        text_f += '%.2f ' % self.event.get("Overtake", 0.0)
        text_f += '%.2f ' % self.event.get("Overtaken", 0.0)
        text_f += '%.2f ' % self.event.get("Head", 0.0)
        text_f += '%.2f ' % self.event.get("Pass", 0.0)
        text_f += '%.2f ' % self.event.get("Surround", 0.0)
        # individual onomatope ID1
        text_f += '%.2f ' % self.onomatodict1.get("FunwaFunwa", 0.0)
        text_f += '%.2f ' % self.onomatodict1.get("FuwaFuwa", 0.0)
        text_f += '%.2f ' % self.onomatodict1.get("YuuraYuura", 0.0)
        text_f += '%.2f ' % self.onomatodict1.get("YuraYura", 0.0)
        text_f += '%.2f ' % self.onomatodict1.get("FuuraFuura", 0.0)
        text_f += '%.2f ' % self.onomatodict1.get("FuraFura", 0.0)
        text_f += '%.2f ' % self.onomatodict1.get("GuraGura", 0.0)
        text_f += '%.2f ' % self.onomatodict1.get("GataGata", 0.0)
        text_f += '%.2f ' % self.onomatodict1.get("BuruBuru", 0.0)
        # individual onomatope ID2
        text_f += '%.2f ' % self.onomatodict2.get("FunwaFunwa", 0.0)
        text_f += '%.2f ' % self.onomatodict2.get("FuwaFuwa", 0.0)
        text_f += '%.2f ' % self.onomatodict2.get("YuuraYuura", 0.0)
        text_f += '%.2f ' % self.onomatodict2.get("YuraYura", 0.0)
        text_f += '%.2f ' % self.onomatodict2.get("FuuraFuura", 0.0)
        text_f += '%.2f ' % self.onomatodict2.get("FuraFura", 0.0)
        text_f += '%.2f ' % self.onomatodict2.get("GuraGura", 0.0)
        text_f += '%.2f ' % self.onomatodict2.get("GataGata", 0.0)
        text_f += '%.2f ' % self.onomatodict2.get("BuruBuru", 0.0)
        #
        text_f += '\n'
        self.file.write(text_f)
        self.time_file = copy.copy(self.time_event)
...
class Apply4:
    def __init__(self, memory_space, idnet, projectID, fields):
        ...
        self.aq_datas_list = []     # メモリ空間から得たデータの置き場
        self.works = []
        self.works_re = []   # 2022June9 追加 ファイルに書き出す。GRファクトリー専用
        ...
        memory_space.set_aq_conditions(self.data_aqcuire_conditions)
        self.file = open('recordGRb.txt', 'a')
        self.DATA_RETAIN_TIME = 5.0 # 2022June20追加。ワークの保持時間と取得するデータの古さを整合する必要。
    ...
    def print_ID(self, work, x, radius, v, indicateID, attribute=[], frame=0):
        f_text = 'frame=%d, time=%.2f, ' % (frame, work.time_org)
        text = 'ID:' + str(work.data_orgID)
        f_text += text + ', x=%.2f, y=%.2f, r=%.2f, Vx=%.2f, Vy=%.2f' % (x[0], x[1], radius, v[0], v[1])
        if attribute != []:
    ...
    # メモリー空間とのやりとりも含めたプロセス本体。
    def app_main4(self, frame, status):
        # データ置き場を空にする
        self.aq_datas_list = []
        ...
        # self.aq_datas_list は、Data_On_Memory形式データのリストになってるはず
        for data in self.aq_datas_list:
            if data.data_type == "event": # 2022Mar15追加。App7の出力
                #for event in data.data:
                new_work_created = True
                for work in self.works :
                    if data.dataID == work.data_orgID: # ここでは、点群の評価としてのイベントのみ表示するので、一致しなければ破棄する
                ...
                # 2022Apr8 追加
                if new_work_created:
                    if len(data.participant_IDs) >= 2 and (data.time_stamp+self.DATA_RETAIN_TIME-1.) > time_g(): # とりあえず、2点間の関係だけ扱う
                        ...
                        if ID1_found and ID2_found:
                            ...
                            self.works.append(new_work)
                            # 2022June21 追加。ファイル名を変える
                            file_head = '%d%d_' % (work1.point.attribute[0], work2.point.attribute[0])
                            if work1.point.x[0] < 650. and work2.point.x[0] < 650. :
                                file_head += 'l_'
                            elif work1.point.x[0] > 800. and work2.point.x[0] > 800. :
                                file_head += 'r_'
                            elif work1.point.x[0] < 800. and work1.point.x[0] > 650. and work2.point.x[0] < 800. and work2.point.x[0] > 650.:
                                file_head += 'm_'
                            else:
                                file_head += 'x_'
                            # 2022June9 追加。新イベント検出時にファイル書き出し用のワーク生成。GRファクトリー専用
                            new_work_re = Work_in_app4_relation(data.time_stamp, data.dataID, work1.point.ID, work2.point.ID, work1.point, work2.point, self.idns, file_head, frame)
                            new_work_re.event = data.data
                            self.works_re.append(new_work_re)
            if data.data_type == "point_cloud_small_num":
                for point in data.data.points: # データが少数点群である前提
                    new_work_created = True
                    for work in self.works :
        ...
        # 5.秒以上更新されなかったら、そのworkを除去する
        remove_unused_works(self.DATA_RETAIN_TIME, self.works)
        ...
        for work in self.works:
            if work.data_type=="Single_point": 
                ...
                self.ax.scatter(work.point.x[0],work.point.x[1],c = 'none', edgecolors=work.color, alpha=1.0, s=size, marker=work.mark)
                self.ax.set_xlim(0,1820)
                self.ax.set_ylim(0,720)
                # 速度ベクトルの描画
                v = np.dot(1.,work.point.v)
                x_list = [work.point.x[0], work.point.x[0]+v[0]]
                y_list = [work.point.x[1], work.point.x[1]+v[1]]
                self.ax.plot(x_list, y_list, color = work.color)
                # IDの記述
                self.print_ID(work, work.point.x, work.point.radius, work.point.v, True, work.point.attribute, frame)
                ...
                #self.max_scale(x, r)
                if work.data_orgID == work.cloud_orgID: # 元のIDが検出できなかった場合はIDを表示する
                    self.print_ID(work, x, r, [0.,0.], True, [], frame)
                else:
                    self.print_ID(work, x, r, [0.,0.], False, [], frame)
        ...
        # 2022June9 関係性をファイルに書き出すためのもの。GRファクトリー専用
        for work_re in self.works_re:
            work_re.renew_relation(self.works)
        for work_re in self.works_re:
            if work_re.time_event > work_re.time_file:
                work_re.write_event(frame)
                if status == False: # 2022July1追加
                    work_re.file.close()
            #if time_g() > (work_re.time_org + 10.):
            #    work_re.file.close()
        # 5.秒以上更新されなかったら、そのworkを除去する
        remove_unused_works(self.DATA_RETAIN_TIME, self.works_re, file=True)
...
class Work_in_app6:
    ...
    def __init__(self, time_org, point, ID, idns, work_origin_ID, fields, fieldID):
        ...
        self.cogObj.STANDARD_SPEED = field.ref_speed # 今回の速度スケールに合わせたモノ
        self.cogObj.A_STANDARD = field.ref_speed     # 今回の速度スケールに合わせたモノ
...
class Work_in_app7:
    ...
    def __init__(self, time_org1, point1, ID1, work_origin_ID1,     \
        ...
        self.cogObj1.STANDARD_SPEED = field.ref_speed # 今回の速度スケールに合わせたモノ
        self.cogObj2.STANDARD_SPEED = field.ref_speed # 今回の速度スケールに合わせたモノ
        self.cogObj1.A_STANDARD = field.ref_speed     # 今回の速度スケールに合わせたモノ
        self.cogObj2.A_STANDARD = field.ref_speed     # 今回の速度スケールに合わせたモノ
        self.cogRelation = cog.Relation(self.cogObj1, self.cogObj2, self.filter_time, time_g())
        self.cogRelation.STANDARD_SPEED = field.ref_speed # 今回の速度スケールに合わせたモノ  # 2022June10
        self.cogRelation.REF_DISTANCE = field.ref_distance  # 2022June2
        self.cogRelation.SPACE_RESOLUTION = field.space_resolution # 2022June10
    ...
    def app_main7(self):
        ...
        for pointA in points:
            for pointB in points:
                #if pointA["point"].ID > pointB["point"].ID:  # 仕様変更2022June17。常にクラスNo.が大きい方を先に（GR用に人をpoint1に）
                if pointA["point"].attribute[0] > pointB["point"].attribute[0]:
                    point1 = pointA
                    point2 = pointB
                else:
                    point1 = pointB
                ...
                new_work_created = True
                for work in self.works :
                    # 仕様変更2022June17 並びが逆の場合も考慮
                    if (point1["point"].ID == work.data_orgID1 and point2["point"].ID == work.data_orgID2) or \
                       (point1["point"].ID == work.data_orgID2 and point2["point"].ID == work.data_orgID1) : # 現有ID一致しなければ、new_work_createdがON・・・一つの例
...
if __name__ == "__main__":
    ...
    while(True): # 本当は、タイマー機能でやる。後ほど
        ...
        apply4.app_main4(frame, status) # 2022June20 場所を移した。
