...
# 2022Apr14 Ver.2 関係性が2つのディクショナリになっていたのを、統合して出力する仕様に変更。
...
class Detected_Object:
    def __init__(self, point, filter_time, time_now=False):
        if time_now==False:
            time_now=time.time()
    ...
    def objects_kinesis(self, point, time_now):
        self.point = point  # 更新が欠けていたので訂正 2022Apr28
    ...
    def objects_kinesis2(self, point, time_now):
        self.point = point  # 更新が欠けていたので訂正 2022Apr28
...
# 2022June23 ver3の追加内容
# ・能動性("Agency")、受影性（"Affectedness"）を、DiffVaVbnorによる演算に変更                     
#   DiffVaVbnor はAとBのどちらが動いているかの指標（単に速度ノルムで判定）
# ・向かう("Head")、過ぎる("Pass")を追加。
#   演算は単に DeltaXDeltaVnor で、対抗具合、または離れる具合
# 2022July5 ver3の追加内容
# ・取り巻く("Surround")を追加
#   演算は 1-DeltaXDeltaVnor**2 で、相対位置と相対速度の直角具合
# 今後の方策
# ・relation_matrix を使った線形演算は馬鹿馬鹿しいのでやめようと思う
# ・「妨害する」も実装が必要で、加速度演算が必要
class Relation:
    def calc_relative_motion(self):
        npxa = np.array(self.objectA.point.x)
        npxb = np.array(self.objectB.point.x)
        ...
        self.Vanor = np.linalg.norm(self.VAfilt)
        self.Vbnor = np.linalg.norm(self.VBfilt)
        self.VdotVnor = np.dot(self.VAfilt, self.VBfilt)/(max(self.Vanor, self.Vbnor, AVOID_ZERO)**2) # 正規化した速度の内積（運動の平行度合い）#2022June29 **2をつける修正
        ...
        self.DiffVaVbnor = (self.Vanor - self.Vbnor)/max((self.Vanor + self.Vbnor), AVOID_ZERO)          # どちらが良く動いているか？ 2022June23追加
...
#       VdotVnor,   DeltaXMeanVnor, 1-DeltaXMeanVnor2, DeltaXDeltaVnor, 1-DeltaXDeltaVnor2, Contact,  VdotVnorl,   DeltaXMeanVnorl, 1-DeltaXMeanVnorl, DeltaXDeltaVnorl, Contactl 
#       ぶつかり合う
...
#       遅れる
        [0.33,       0.33,           0.,               -0.33,             0.,               0.,        0.,          0.,              0.,                0.,              0.],\
#       追いつく
        [0.33,       0.33,           0.,                0.33,             0.,               0.,        0.,          0.,              0.,                0.,              0.],\
#       会う
...
#       取り巻く 2022July5追加  
    ...
    def cogni_vec2dict(self, cogni_vector):
        cogni_dict = {  "Hit":       cogni_vector[0], \
                        ...
                        "Head":      cogni_vector[13], \
                        "Pass":      cogni_vector[14], \
                        "Surround":  cogni_vector[15], \
                              }
        return cogni_dict
    ...
    def __init__(self, objectA, objectB, filter_time, time_now=False):
        if time_now==False:
            time_now=time.time()
        ...
        self.ID = ID_NOT_ASSIGNED
        #self.IDa = objectA.point.ID
        #self.IDb = objectB.point.ID
        #self.fieldID = fieldID
        #self.categoryIDs = []     
        #self.V = []         # np.array 上記の微分
        self.VAfilt = np.array(self.objectA.point.v)   # フィルターをかけた速度。初期値はフィルタなしで入れる
        self.VBfilt = np.array(self.objectB.point.v)   # フィルターをかけた速度。初期値はフィルタなしで入れる
        #self.Vanor = np.linalg.norm(self.VAfilt)   # calc_relative_motion()で演算してるので不要
        #self.Vbnor = np.linalg.norm(self.VBfilt)   # calc_relative_motion()で演算してるので不要
        self.Contact = False
...
#                   意図性        ・・・アルゴリズム未作成                      
                    "Volitionality": 0.0,\
#                   能動性         ・・・DiffVaVbnorに変更 2022June23
                    "Agency":       0.0,\
#                   受影性         ・・・-DiffVaVbnorに変更 2022June23
                    "Affectedness": 0.0,\
#                   連動性（フカダ用語）
                    "Linkageness":  0.0,\
#                   一体性（フカダ用語）
...
#                   類似性（フカダ用語）・・・アルゴリズム未作成
        ...
        #self.MIN_DISTANCE_TO_IDENTICAL_OBJECT = 0.7 # m
        #self.TIME_RESOLUTION = 0.05                 # sec 認知の時間分解能 
        self.REF_DISTANCE = 2.0                     # m この距離より小さい距離での相互作用は強いとする
        self.SPACE_RESOLUTION = 0.2                 # m
        #self.VMIN = 0.001                           # m/sec 速度の認知限界
        #self.REF_SPEED = 0.3                        # m/sec
        self.STANDARD_SPEED = 0.4                   # m/sec
    ...
    # 入力仕様を修正。リンク切れ起こすとポイントデータが更新されない
    def relation_evaluation(self, objectA, objectB, time_now):
        # データ更新。これをしない為に演算してなかった 2022Apr28修正
        self.objectA = objectA
        self.objectB = objectB
        #self.pointA = objectA.point
        #self.pointB = objectB.point
        #self.relation["Time"] = time_now # 廃止
        filter_ratio = min(1., (time_now - self.last_time)/self.filter_time)
        self.VAfilt = (1. - filter_ratio)*self.VAfilt + filter_ratio*np.array(self.objectA.point.v) # フィルターをかけた速度
        self.VBfilt = (1. - filter_ratio)*self.VBfilt + filter_ratio*np.array(self.objectB.point.v) # フィルターをかけた速度
        self.calc_relative_motion()
        # 仕様変更。速度の係数をかける 2022June2 # 係数に上限設定 2022Jul5 
        MAX_V_FACTOR = 1.3
        cogni_vector = np.dot(self.relation_matrix, self.state_vector()) * min(MAX_V_FACTOR, (max(self.Vanor, self.Vbnor)/self.STANDARD_SPEED))
        max_val = (max(0., np.amax(cogni_vector)))**2
        self.cogni_dict = self.cogni_vec2dict(cogni_vector)
        self.relation["Unity"]  = (max(0., self.cogni_dict["Co_motion"]))**2
        self.relation["Agency"]       = max(0., self.DiffVaVbnor) #(max(0., self.cogni_dict["Chase"]))**2 # 2022June23仕様変更
        self.relation["Affectedness"] = max(0.,-self.DiffVaVbnor) #(max(0., self.cogni_dict["Chased"]))**2 # 2022June23仕様変更
        self.relation["Kinesis"] = (max(0., self.cogni_dict["Sideby"]))**2
        self.set_last()
        #self.cogni_dict = cogni_dict

        contactDistance = max(self.SPACE_RESOLUTION, self.objectA.point.radius + self.objectB.point.radius)
        contact = (contactDistance - self.Distance)/contactDistance
        #if self.Distance < max(SPACE_RESOLUTION, self.objectA.point.radius + self.objectB.point.radius):
        if contact > 0.:
            self.Contact = True
        ...
        total_relation = {'Contact': contact}
        total_relation.update(self.relation)
        total_relation.update(self.cogni_dict)
        ...
        self.last_time = time_now
        return total_relation   # ここがVer.2の大きな変更点
...
def plot_person(detected_objs, relations, fig, ax):
    ...
    for relation in relations :
        if relation.Attention > 0.5 :
            xa = relation.objectA.point.x[0]
            ya = relation.objectA.point.x[1]
            xb = relation.objectB.point.x[0]
            yb = relation.objectB.point.x[1]
...
def main():
    ...
    for n in range(200):
        ...
        for obj in detected_objs:
            # リンク切れしない限りこれで良い 2022Apr28
            obj.objects_kinesis(obj.point, time.time())
    
        for rel in relations:
            # リンク切れしない限り、これで良いはず 2022Apr28
            cog = rel.relation_evaluation(rel.objectA, rel.objectB, time.time())
            # "Sideby"の表示
            if rel.Attention > 0.5:
                for cogkey in cog.keys():
                    if max(0., cog[cogkey])**2 == rel.Attention:
                        s = cogkey
                #print("Attention in ID %d and %d %s val=%.2f" % (rel.objectA.point.ID, rel.objectB.point.ID, s, rel.Attention))
                if s == "Sideby":
