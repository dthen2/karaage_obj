...
class Work_in_app4_relation: # 2022June9 関係性をファイルに書き出すためのもの。GRファクトリー専用
    def __init__(self, time_org, eventID, ID1, ID2, point1, point2, idns, frame):
        self.time_event = time_org
        self.time_eval = -1
        self.time_point = -1
        filename = 'ai_r' + '%d_%d_%d' % (frame, ID1, ID2)
        filename += '.txt'
    ...
    def renew_relation(self, works):
        for work in works:
            ...
            elif work.data_type == "evaluation" and self.time_eval < work.time_org:
                if self.ID1 == work.data_orgID:
                    self.onomatodict1 = work.onomatodict
                    self.time_eval = work.time_org
                elif self.ID2 == work.data_orgID:
                    self.onomatodict2 = work.onomatodict
                    self.time_eval = work.time_org
            elif work.data_type == "Single_point" and self.time_point < work.time_org:
                if self.ID1 == work.data_orgID:
                    self.point1 = work.point
                    self.time_point = work.time_org
                elif self.ID2 == work.data_orgID:
                    self.point2 = work.point
                    self.time_point = work.time_org
        self.time_org = max(self.time_event, self.time_eval, self.time_point)   # ここ悩ましい
                ...
                if new_work_created:
                    if len(data.participant_IDs) >= 2 and (data.time_stamp+self.DATA_RETAIN_TIME-1.) > time_g(): # とりあえず、2点間の関係だけ扱う
                        ...
                        if ID1_found and ID2_found:
                            ...
                            new_work_re = Work_in_app4_relation(data.time_stamp, data.dataID, work1.point.ID, work2.point.ID, work1.point, work2.point, self.idns, frame)
...
if __name__ == "__main__":
    ...
    while(True): # 本当は、タイマー機能でやる。後ほど
        ...
        apply4.app_main4(frame) # 2022June20 場所を移した。
