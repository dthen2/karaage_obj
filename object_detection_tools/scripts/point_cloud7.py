...
class Closest:
    ...
    def closest_point_diff(self, x_scale, mag_scale):
        ...
        #diff_rot = []
        diff_x = []     # 微分係数
        diff_mag = np.eye(self.dimension)   # 微分係数
        #diff_rot_m = []
        shift_best = []
        mag_best = np.eye(self.dimension)
        ...
        for n in range(self.dimension):
            ...
            if matching_func2==matching_func: # ゼロ割防止
                ...
                c = (matching_func_norm-matching_func2)*x_scale/(matching_func2-2.*matching_func+matching_func_norm)/2.
                shift_best.append(x_scale + c)
                nonzero_sift = True
            self.sift[n] = sift_prev[n]

            for m in range(self.dimension):
                self.transform[n,m] = self.transform[n,m] + mag_scale
                matching_func = self.closest_point_evaluation()
                self.transform[n,m] = self.transform[n,m] + mag_scale
                matching_func2 = self.closest_point_evaluation()
                if matching_func2==matching_func: # ゼロ割防止
                    ...
                    mag_best[n,m] = (mag_scale + c)
                    #print('hoge')
                    nonzero_mag = True
                self.transform = copy.copy(mag_prev)
        # 新アルゴリズム ピーク位置推定
        # shift_best, mag_best は共にピーク位置の推定値だが、おかしな値の可能性がある
        ...
        if nonzero_sift:
            diff_x_scale   = min(x_scale,   np.linalg.norm(np.array(shift_best)))
            shift_best_diff = np.array(diff_x)/np.linalg.norm(diff_x)*diff_x_scale
        else:
            diff_x_scale   = x_scale
            shift_best_diff = np.array(diff_x)
        if nonzero_mag:
            diff_mag_scale = min(mag_scale, np.linalg.norm(mag_best))
            mag_best_diff = mag_best#np.array(diff_mag)/np.linalg.norm(diff_mag)*diff_mag_scale
        else:
            diff_mag_scale = mag_scale
            mag_best_diff = np.array(diff_mag)
        
        return shift_best_diff, mag_best_diff


    def closest_point_approach(self, matching_func_best, shift_best_diff, mag_best_diff, tolerance=0.0):
        best = matching_func_best*(1.0 + tolerance)
        x_scale   = np.linalg.norm(shift_best_diff)
        mag_scale = np.linalg.norm(mag_best_diff)

        sift_pre = copy.copy(self.sift)
        self.sift = self.sift + shift_best_diff
        ...
        self.transform = self.transform + mag_best_diff
    ...
    def closest_gradient_approach(self, x_scale, mag_scale, number, tolerance=0.0):
        ...
        for converge_num in range(number):
            shift_best_diff, mag_best_diff = self.closest_point_diff( x_scale, mag_scale)
            mat, is_converge_sift, is_converge_mag, x_scale, mag_scale = self.closest_point_approach( mat, shift_best_diff, mag_best_diff, tolerance)
...
def main():
    ...
    resolution_init = 1.
    resolution_final = 1.
    closest.resolution = resolution_init
    ...
    closest.resolution = resolution_final
    ...
    #    if is_converge:
    #        break
    converge_num, mat, is_converge_sift, is_converge_mag = closest.closest_gradient_approach(x_scale, mag_scale, number=30, tolerance=0.0)
    plt.cla()
    #plotdata(ax,closest)
    plotdata_anti(ax,closest) # refの方を座標変換
    fig.show()
...
def main1b():
    ...
    mat_prev = 0.
    for i in range(10):
        mat = closest.closest_point_search( search_num=5, matching_func_best=mat, transmode='rot')
        if mat > mat_prev:
            plt.cla()
            #plotdata(ax,closest)
            plotdata_anti(ax,closest) # refの方を座標変換
            fig.show()
            plt.pause(0.1) #これが無いと何故か表示されない
            print('at i=%d, matching = %f'%(i, mat))
        mat_prev = mat
    ...
    converge_num, mat, is_converge_sift, is_converge_mag = closest.closest_gradient_approach(x_scale, mag_scale, number=1, tolerance=0.0)
    plt.cla()
    #plotdata(ax,closest)
    plotdata_anti(ax,closest) # refの方を座標変換
    fig.show()
