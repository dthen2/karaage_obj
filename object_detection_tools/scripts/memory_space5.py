...
def aq_condition_match(aq_cond, data):
    ...
    and _scan_IDs(aq_cond.get("aq_work_IDs",[]), data.work_ID) \
...
#       欲しいデータのデータタイプ。文字列のリスト。空リストなら不問
    ...
    def librarian_scanner(self):
        ...
        for upload_data in self.upload_datas: 
            ...
            for n in range(len(self.memory_space)):
                ...
                if self.memory_space[n].data_ID == upload_data.Attention_ID:
    ...
    def librarian_swapper_before_scanner(self):
        ...
        for rep in self.aq_conditions_rep:
            for n in reversed(range(len(self.aq_conditions))):
                if self.aq_conditions[n]["app_ID"] == rep[0]: # rep[0]はappID
    ...
    # app_IDの一致する要求データ仕様を全て削除して入れ替える
    #def renew_aq_conditions(self, app_ID, aq_condition_list_new):
    #    for n in reversed(range(len(self.aq_conditions))):
    #        if self.aq_conditions[n]["app_ID"] == app_ID:
    #            del self.aq_conditions[n]
    ...
    # app_IDの一致する要求データ仕様を全て削除して入れ替える
    def renew_aq_conditions(self, app_ID, aq_condition_list_rep):
        self.aq_conditions_rep += [app_ID, aq_condition_list_rep]
